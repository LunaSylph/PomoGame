...
```json
{
  ...
  "grid": {
    "size": 5,
    "tiles": [
      { "id": "0-0", "state": "closed", "isSpecial": false }
    ]
  },
  "session": {
    "phase": "idle",
...
- **grid.tiles**: 25 elemanlı dizi (5x5). `state`: "closed" | "cleared". `isSpecial`: true olan 2 tile, Lumbermill/Mine blueprint konumları (Bölüm 5 — başlangıca bitişik, sabit).
